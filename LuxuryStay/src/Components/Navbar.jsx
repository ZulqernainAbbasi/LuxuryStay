    ...
    return (
        <nav className={`fixed top-0 left-0 w-full flex items-center justify-between px-4 md:px-16 lg:px-24 xl:px-32 transition-all duration-500 z-50 ${isScrolled ? "bg-white/80 shadow-md text-gray-700 backdrop-blur-lg py-3 md:py-4" : "py-4 md:py-6"}`}>
            ...
            <div className="hidden md:flex items-center gap-4 lg:gap-8">
                ...
                {user && (
                    <button
                        className={`border px-4 py-1 text-sm rounded-full transition-all ${isScrolled ? 'text-black' : 'text-white'}`}
            ...
            <div className="hidden md:flex items-center gap-4">
                ...
                {user ? (
                    ...
                    <button
                        onClick={openSignIn}
                        className={`px-8 py-2.5 rounded-full transition-all ${isScrolled ? "text-white bg-black" : "bg-white text-black"}`}
                    >
                        Login
                    </button>
                )}
            ...
            {/* Mobile Menu */}
            <div className={`fixed top-0 left-0 w-full h-screen bg-white flex flex-col items-center justify-center gap-6 transition-all duration-500 ${isMenuOpen ? "translate-x-0" : "-translate-x-full"}`}>
